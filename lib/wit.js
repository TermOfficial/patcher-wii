...
class WIT {
    ...
    async extract(format, inputPath, outputPath) {
        ...
        try {
            const { stdout, stderr } = await childProcess.exec(this.wit, `EXTRACT "${path.normalize(inputPath)}" -d "${path.normalize(outputPath)}"`);
    ...
    async copy(format, inputPath, outputPath, isWbfs = false) {
        ...
        try {
            const { stdout, stderr } = await childProcess.exec(this.wit, `COPY ${isWbfs ? "--wbfs" : ""} "${path.normalize(inputPath)}" "${path.normalize(outputPath)}"`);
            if (stderr) throw stderr;
